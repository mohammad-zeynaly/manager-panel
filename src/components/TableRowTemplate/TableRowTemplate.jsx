...
const TableRowTemplate = ({ id, img, name, price, fieldPrice }) => {
  const dispatch = useDispatch();
  const isDelete = useSelector((state) => state.adminPanel.isDelete);
  const [deleteProductInput, setDeleteProductInput] = useState();

  const deleteHandler = (id, imgUrl) => {
    dispatch(modalStatusHandler(true));
    setDeleteProductInput({ id, imgUrl });
  };

  useEffect(() => {
    if (isDelete) {
      dispatch(deleteProduct(deleteProductInput));
    }
  }, [isDelete]);
      ...
      <tr className="table-body__row">
        ...
        <td className="table-body__column">
          <div className="table-body__column-actions">
            <Link
              onClick={() =>
                dispatch(mainEditProductHandler({ id, name, price, img }))
              }
              to="/update-product"
